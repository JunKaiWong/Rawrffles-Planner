...
-- link_photos shipped holding only Telegram file_ids. Mini App uploads never
-- reach Telegram - the bot must not post the couple's photos to their own group
-- - so those rows carry bytes instead, and file_id has to become nullable.
ALTER TABLE link_photos ADD COLUMN IF NOT EXISTS image_data   BYTEA;
ALTER TABLE link_photos ADD COLUMN IF NOT EXISTS content_type TEXT;
ALTER TABLE link_photos ADD COLUMN IF NOT EXISTS digest       TEXT;
ALTER TABLE link_photos ALTER COLUMN file_id DROP NOT NULL;
